...
public class ProductService {
	...
	public ResponseEntity<?> addProduct(Product product){
		...
		message.setSkuCode(product.getSkuCode());
		message.setDate(new Date());
		message.setUpdateType(UpdateType.CREATE);
		
		template.convertAndSend(ProductUpdatesMessageConfig.INVENTORY_EVENTS_EXCHANGE, 
								ProductUpdatesMessageConfig.PRODUCT_CREATED_ROUTING_KEY,
								message);
